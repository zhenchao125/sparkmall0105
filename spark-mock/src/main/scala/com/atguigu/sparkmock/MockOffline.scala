...
object MockOffline {
    ...
    def mockUserVisitAction: List[UserVisitAction] = {
        ...
        // 开始日期
        val fromDate = dateFormatter.parse("2019-06-12")
        // 结束日期
        val toDate = dateFormatter.parse("2019-06-15")
            ...
            while (!isQuit) {
                ...
                if (action == "quit") {
                    ...
                    action match {
                        ...
                        case "order" => {
                            orderCategoryIds = RandomNumUtil.randomMultiInt(1, cargoryNum, RandomNumUtil.randomInt(1, 3), false).mkString(",")
                            orderProductIds = RandomNumUtil.randomMultiInt(1, productNum, RandomNumUtil.randomInt(1, 5), false).mkString(",")
                        }
                        case "pay" => {
                            payCategoryIds = RandomNumUtil.randomMultiInt(1, cargoryNum, RandomNumUtil.randomInt(1, 3), false).mkString(",")
